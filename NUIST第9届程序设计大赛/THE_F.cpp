...
我们还是来计算x的11次方。我们将11用2的指数形式加权来表示，那么11 = 23×1 + 22×0 + 21×1 + 2?×1。你可能已经发现了，2的指数的权值连起来就是11的二进制表示：1011。这样，我们要计算的x^11就可以转化为x^(2?) × x^(21) × x^(23)，原来要乘11次的方法被我们缩减到了3次。
...
位运算是计算机中非常强大的工具，x *= x一句就能在循环中不断地计算出x^(2?), x^(21), x^(22), ... 在n & 1的判断处，如果n的二进制末位是1，那res *= x就会被执行，x^(2^y) （y为这个循环执行的次数）就会被乘进结果里，x^n就会被很快计算出来。这个算法的时间复杂度是O(log(n))。这种算法也被称作快速幂。
...
当我们要计算一个矩阵A的幂时，也可以用快速幂算法，只需要把上方计算x^n的值的代码中res *= x和x *= x两句替换成矩阵的乘法即可。

矩阵快速幂的用处很广泛，其中一项就是求斐波那契数列的第n项值。众所周知，斐波那契数列的递推公式为：f(n) = f(n-1) + f(n-2)。因此，它也可以用矩阵表示。
图片描述
