...
const string habb_Mouhth[]    = { "pop",  "no",   "zip",   "zotz",  "tzec", "xul", "yoxkin",
                               "mol",  "chen", "yax",   "zac",   "ceh",  "mac", "kankin",
                               "muan", "pax",  "koyab", "cumhu", "uayet" };
const string Tzolkin_Mounth[] = { "imix",  "ik",    "akbal", "kan",   "chicchan", "cimi", "manik",
...
int          main(void)
{
    ...
    while(std::cin >> n)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            std::cout << day % Tzoljin_year % 13 + 1 << ' '
                      << Tzolkin_Mounth[day % Tzoljin_year % 20] << ' ' << day / Tzoljin_year
                      << std::endl;
        }
    }
#ifdef DEBUG
