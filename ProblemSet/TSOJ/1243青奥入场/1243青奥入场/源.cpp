...
template <typename T>
void buble_sort(T *from, T *After_To)
{
...
int main(void)
{
    ...
    while(scanf("%d", &n) != EOF)
    {
        for(int i = 0; i < n; i++)
        {
            std::cin >> countries[i].No >> countries[i].name >> countries[i].NumOfAthe >>
              countries[i].state;
        }
        ...
        for(int i = 0; i < n; i++)
        {
            std::cout << countries[i].No << ' ' << countries[i].name << ' '
                      << countries[i].NumOfAthe << ' ' << countries[i].state << std::endl;
        }
    }
    return 0;
}
