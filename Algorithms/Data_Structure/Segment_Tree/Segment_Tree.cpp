...
Segment_Tree::Segment_Tree(size_t size, T def) /*:tree(new Node[size*4])*/
{
    ...
    stack.push(
      std::pair<size_t, std::pair<size_t, size_t> >(1, std::pair<size_t, size_t>(0, size)));
    while(!stack.empty())
    {
        std::pair<size_t, std::pair<size_t, size_t> > cur = stack.top();
        stack.pop();
        ...
        if(from != to)
        {
            ...
            stack.push(
              std::pair<size_t, std::pair<size_t, size_t> >(pos << 1,
                                                            std::pair<size_t, size_t>(from, mid)));
            tree[pos].r = &tree[pos << 1 | 1];
            stack.push(std::pair<size_t, std::pair<size_t, size_t> >(
              pos << 1 | 1,
              std::pair<size_t, size_t>(mid + 1, to)));
        }
    }
}
...
// template<class t>
Node::Node(size_t from, size_t to):
    low(from),
    up(to),
    l(NULL),
    r(NULL),
...
// Node & Node ::left()
//{
//	// TODO: 闂侀潻璐熼崝宥夘敆濠靛洤绶為柛鏇ㄥ墮缁插鏌涜箛銉﹀ return
//闁荤姴娴傞崢鑹般亹閿燂拷
//}
////template<class T>
// Node & Node ::right()
