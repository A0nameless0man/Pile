...
template <class T>
P_Queue<T>::P_Queue(size_t minCompacity)
{
    Heap          = NULL;
    compacity     = 0;
    compacityUsed = 0;
...
template <class T>
P_Queue<T>::~P_Queue(void)
{
    delete Heap;
}
template <class T>
void P_Queue<T>::insert(const T &target)
{
...
template <class T>
T P_Queue<T>::top(void) const
{
    return Heap[0];
}
template <class T>
void P_Queue<T>::pop(void)
{
...
template <class T>
size_t P_Queue<T>::size(void) const
{
    return compacityUsed;
}
template <class T>
bool P_Queue<T>::empty(void) const
{
    return size() == 0;
}
template <class T>
void P_Queue<T>::matainHeap(size_t target)
{
    size_t next = target;
    while(true)
    {
...
template <class T>
size_t P_Queue<T>::changeSizeTo(size_t target)
{
...
template <class T>
inline void P_Queue<T>::moreCompacity(void)
{
    changeSizeTo(2 * compacity);
}
template <class T>
inline size_t P_Queue<T>::left(size_t i) const
{
    size_t ans = i * 2;
...
template <class T>
inline size_t P_Queue<T>::right(size_t i) const
{
    size_t ans = i * 2 + 1;
    if(ans < compacityUsed)
    {
...
template <class T>
inline size_t P_Queue<T>::parent(size_t i) const
{
    return i / 2;
}
}  // namespace P_Queue
