...
int main(void)
{
	while (scanf("%d",&p)!=EOF)
	{
		...
		//	{
		//		mem_DP[i][odd] = max(mem_DP[i - 1][odd], mem_DP[i - 1][!odd] + (odd ? potions[i] : -potions[i]));
		//	}
		//}//end loop
		//printf("%d\n", result);
		printf("%d\n", oddDP(p - 1, true));
...
int oddDP(int n,bool isodd)
{
	...
	else
	{
		...
		else
		{
			...
			//}
			//return mem_DP[n][odd] =( result + (isodd ? potions[n] : -potions[n]));//是我沙雕，转移方程写复杂了。
			mem_DP[n][odd] = max(oddDP(n - 1, isodd), oddDP(n-1,!isodd) + (isodd ? potions[n] : -potions[n]));
			//if (isodd)
			//{
