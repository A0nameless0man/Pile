...
KeyWordType::BasicType KeyWordType::stringToBasicType(const std::string &name)
{
    ...
    std::unordered_map<std::string, BasicType> map = { { "name", Type::name },
                                                       { "id", Type::id },
                                                       { "sex", Type::sex },
                                                       { "class", Type::stuClass },
                                                       { "grade", Type::stuGrade } };
...
IDVec WhereFilter::filt(const StudentList &list)
{
    try
    {
        ...
        std::unordered_map<std::string, LogicOp> ops = { { "and", logicAnd },
                                                         { "or", logicOr },
                                                         { "(", left },
                                                         { ")", right } };
        std::stack<IDVec>                        IDs;
        ...
        while(cmd >> buf)
        {
            if(!ops.contains(buf))
            {
                ...
                if(!Cmap.contains(op))
                {
                    throw std::invalid_argument("UnKnown opewrator " + op + " in express\n" +
                                                keyName + " " + op + " " + keyVal);
                }
                else
                {
                    ...
                    switch(p)
                    {
                        case eq:
                            ...
                            break;
                        case neq:
                            IDs.push(list.getByKey<std::not_equal_to<KeyWord>>(
                              keyName,
                              KeyWord(KeyWordType(keyName), keyVal)));
                            break;
                        case ge:
                            IDs.push(list.getByKey<std::greater<KeyWord>>(
                              keyName,
                              KeyWord(KeyWordType(keyName), keyVal)));
                            break;
                        case geq:
                            IDs.push(list.getByKey<std::greater_equal<KeyWord>>(
                              keyName,
                              KeyWord(KeyWordType(keyName), keyVal)));
                            break;
                        case le:
                            IDs.push(list.getByKey<std::less<KeyWord>>(
                              keyName,
                              KeyWord(KeyWordType(keyName), keyVal)));
                            break;
                        case leq:
                            IDs.push(list.getByKey<std::less_equal<KeyWord>>(
                              keyName,
                              KeyWord(KeyWordType(keyName), keyVal)));
                            break;
                        default:
                            throw std::invalid_argument("UnKnown opewrator " + op +
                                                        " after explan express\n" + keyName + " " +
                                                        op + " " + keyVal);
                            break;
                    }
                }
