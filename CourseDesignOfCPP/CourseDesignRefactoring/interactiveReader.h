...
{
public:
    WithIn(const std::set<T, Compare, Allocator> &set        = {},
           const Serializer                       serializer = Serializer());
    bool        operator()(const T &t) const;
    std::string explainRestriction() const;
    void        add(const T &t);
...
public:
    ...
    inline T read(Is &         is,
                  Os &         os,
                  std::string  variableNameRequired,
                  Restriction  restriction = NoRestrict<T>(),
                  UnSerializer unSerlizer  = StdIstreamReader<T>()) const;
};
...
inline InRange<T, Serializer, Compare>::InRange(const T &        lowerBound,
                                                const T &        upperBound,
                                                const Serializer serializer,
                                                const Compare    cmp):
...
{
    return "类型应当为" + std::string(typeid(T).name()) + ",且范围在" + serializer(lowerBound) +
           "到" + serializer(upperBound) + "之间";
}

template <class T, class Serializer, class Compare, class Allocator>
inline WithIn<T, Serializer, Compare, Allocator>::WithIn(const std::set<T, Compare, Allocator> &set,
                                                         const Serializer serializer):
    set(set),
    serializer(serializer)
{
...
template <class T>
template <class Is, class Os, class Restriction, class UnSerializer>
inline T InteractiveReader<T>::read(Is &         is,
                                    Os &         os,
                                    std::string  variableNameRequired,
                                    Restriction  restriction,
                                    UnSerializer unSerlizer) const
{
    while(true)
    {
