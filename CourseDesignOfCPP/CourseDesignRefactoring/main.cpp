...
void brifOnStudent(const Server &server, const Student::LogicID &id)
{
    ...
    for(auto i: server.getCourseSelectionRecordIdByStudentId(id))
    {
        ...
        if(csr.getGrade() >= 60.0)
            costream<os>(GREY) << "\t" << course.getName() << " : \t" << csr.getGrade()
                               << std::endl;
        else
...
void oneLineBrifOfCourse(const Server &server, const Course::CourseID &id)
{
    ...
    costream<os>(YELLOW) << "课程名:\t" << course.getName() << ",\t"
                         << "课时:\t" << course.getClassHour() << std::endl;
}
...
Student::StudentClassLogicalID selectClass(const Server &server)
{
    while(true)
        try
        {
            auto m = iReader::InteractiveReader<std::string>().read(
              is,
              os,
              "选择班级的方法",
              iReader::WithIn<std::string>({ "List", "Name", "Quit" }),
              iReader::StdIstreamReader<std::string>());
            switch(m[0])
            {
                case 'N':
                {
                    auto s = iReader::InteractiveReader<Student::StudentClassName>().read(
                      is,
                      os,
                      "班级的名字",
...
Server::StudentIdSet selectStudent(const Server &server)
{
    while(true)
    {
        auto s = iReader::InteractiveReader<std::string>().read(
          is,
          os,
          "选择学生的条件",
          iReader::WithIn<std::string>({ "ID", "Name", "Class", "Quit" }),
          iReader::StdIstreamStringReader());
        try
        {
            switch(s[0])
            {
                case 'I':
                    return Server::StudentIdSet(
                      { server.getStudentLogicIdByID(iReader::InteractiveReader<Student::ID>().read(
                        is,
                        os,
                        "学生的ID",
                        iReader::NoRestrict<Student::ID>(),
                        iReader::StdIstreamStringReader())) });
                    break;
                case 'N':
                    return server.getStudentLogicIdByName(
                      iReader::InteractiveReader<Student::UserName>().read(
                        is,
                        os,
                        "学生的姓名",
                        iReader::NoRestrict<Student::UserName>(),
                        iReader::StdIstreamStringReader()));
                    break;
                case 'C':
...
Course::CourseID selectCourse(const Server &server)
{
    while(true)
    {
        auto s = iReader::InteractiveReader<std::string>().read(
          ...
          os,
          "选择课程的条件",
          iReader::WithIn<std::string>({ "Name", "List", "Quit" }),
          iReader::StdIstreamStringReader());
        try
        {
            switch(s[0])
            {
                case 'N':
                {
                    auto name = iReader::InteractiveReader<std::string>().read(
                      is,
                      os,
                      "课程的名字",
                      iReader::NoRestrict<std::string>(),
                      iReader::StdIstreamStringReader());
...
Server::CourseSelectionRecordIdSet selectCSR(const Server &server)
{
    while(true)
    {
        auto s = iReader::InteractiveReader<std::string>().read(
          ...
          os,
          "选择选课记录的条件",
          iReader::WithIn<std::string>({ "Both", "Student", "Quit" }),
          iReader::StdIstreamStringReader());
        try
        {
            switch(s[0])
            {
                ...
                case 'B':
                {
                    ...
                    for(auto stu: stuset)
                        ans.insert(
                          server.getCourseSelectionRecordIdByStudentIdAndCourseID(stu, course));
                    break;
                }
                case 'Q':
                    //[[fallthrough]]
...
User inputUser(void)
{
    auto name   = iReader::InteractiveReader<std::string>().read(is,
                                                               os,
                                                               "姓名",
                                                               iReader::NoRestrict<std::string>(),
                                                               iReader::StdIstreamStringReader());
    auto id     = iReader::InteractiveReader<std::string>().read(is,
                                                             os,
                                                             "ID",
                                                             iReader::NoRestrict<std::string>(),
                                                             iReader::StdIstreamStringReader());
    auto gender = iReader::InteractiveReader<std::string>().read(
      is,
      os,
      "性别",
      iReader::WithIn<std::string>({ "Male", "Female", "Unknown" }),
      iReader::StdIstreamStringReader());
    // auto ID= iReader::
...
void addAdmin(Server &server)
{
    ...
    auto pwd  = iReader::InteractiveReader<User::PWD>().read(is,
                                                            os,
                                                            "管理员密码",
...
void addStudent(Server &server)
{
    ...
    server.addStudent(Student(user,
                              theclass,
                              iReader::InteractiveReader<Student::StudentGrade>().read(
                                is,
                                os,
                                "入学年份",
                                iReader::NoRestrict<Student::StudentGrade>(),
                                iReader::StdIstreamReader<Student::StudentGrade>())));
}

template <std::ostream &os = std::cout, std::istream &is = std::cin>
void addClass(Server &server)
{
    server.addClass(iReader::InteractiveReader<Student::StudentClassName>().read(
      is,
      os,
      "班级名称",
      iReader::NoRestrict<Student::StudentClassName>(),
...
int asStudent(const Server &server)
{
    ...
    {
        auto id =
          iReader::InteractiveReader<Student::ID>().read(is,
                                                         os,
                                                         "ID",
                                                         iReader::NoRestrict<Student::ID>(),
                                                         iReader::StdIstreamReader<Student::ID>());
        auto logicID = server.getStudentLogicIdByID(id);
        auto pwd     = iReader::InteractiveReader<Student::PWD>().read(
          is,
          os,
          "密码",
          iReader::NoRestrict<Student::PWD>(),
          iReader::StdIstreamReader<Student::PWD>());
                    ...
                    switch(op[0])
                    {
                        case 'a':
                        {
                            auto tar = iReader::InteractiveReader<std::string>().read(
                              ...
                              os,
                              "操作对象",
                              iReader::WithIn<std::string>({ "student",
                                                             "course",
                        ...
                        case 'd':
                        {
                            auto tar = iReader::InteractiveReader<std::string>().read(
                              ...
                              "操作对象",
                              iReader::WithIn<std::string>(
                                { "student", "CourseSelectionRecord", "csr", "Quit" }),
                              iReader::StdIstreamReader<std::string>());
                            if(tar == "student")
                            {
                                for(auto s: selectStudent(server))
...
int __main__(void)
{
    ...
    auto s = iReader::InteractiveReader<std::string>().read(
      ...
      std::cout,
      "登陆类型",
      iReader::WithIn<std::string>({ "s", "student", "a", "admin" }),
      iReader::StdIstreamStringReader());
    switch(s[0])
    {
        case 's':
...
int test(void)
{
    ...
    myServer.addCourseSelectionRecord(*(myServer.getStudentLogicIdByName("hh").begin()),
                                      myServer.getCourseIdByCourseName("lm"));
    addStudent(myServer);
    selectCourse(myServer);
    for(auto s: selectStudent(myServer))
        brifOnStudent(myServer, s);
    auto s = myServer.getStudentLogicIdByClass(myServer.getClassLogicalIdByClassName("2b"));
