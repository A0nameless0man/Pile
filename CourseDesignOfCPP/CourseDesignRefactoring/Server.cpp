...
#include <cstdlib>
Server::Server(): studentCount(0), courseCount(0), courseSelectionCount(0), classesCount(0)
{
...
{
    auto stull = [](const std::string &str) -> unsigned long long {
        return std::strtoull(str.c_str(), NULL, 0);
    };

#define Auto(i, js, x, y, z)                                 \
    if(js.contains(#x))                                      \
        for(auto i = js[#x].begin(); i != js[#x].end(); i++) \
    x.insert({ (y), (z) })
#define AutoFor(i, js, x) \
    if(js.contains(#x))   \
        for(auto i = js[#x].begin(); i != js[#x].end(); i++)
...
CmdResalt Server::addCourseSelectionRecord(const Student::LogicID &stuId,
                                           const Course::CourseID &course)
{
    ...
    courseSelectionRecords.insert(
      { courseSelectionCount, CourseSelectionRecord(stuId, 0, course) });
...
CmdResalt Server::setPassWordOfStudent(const Student::LogicID &studentId,
...
CmdResalt Server::setCourseGrade(const Student::LogicID &                    stuId,
                                 const Course::CourseID &                    courseId,
                                 const CourseSelectionRecord::GradeOfCourse &grade)
{
    auto iter = courseSelectionIndex.find(stuId);
    RunTimeAssert(iter != courseSelectionIndex.end(),
                  Docs::noCourseSelectionRecordForThisStudentError);
    auto iteriter = iter->second.find(courseId);
    RunTimeAssert(iteriter != iter->second.end(),
                  Docs::noSuchCourseSelectionRecordForThisStudentError);
    courseSelectionRecords.find(iteriter->second)->second.setGrade(grade);
    return CmdResalt(true, std::to_string(1) + Docs::setSuccessNoteSuffix);
}

CmdResalt Server::setCourseGrade(const CourseSelectionRecord::CourseSelectionRecordID csrid,
                                 const CourseSelectionRecord::GradeOfCourse &         grade)
{
...
CmdResalt Server::removeCourseSelectionRecord(const Student::LogicID &stuId,
                                              const Course::CourseID &courseId)
{
    auto iter = courseSelectionIndex.find(stuId);
    RunTimeAssert(iter != courseSelectionIndex.end(),
...
const CourseSelectionRecord::CourseSelectionRecordID
Server::getCourseSelectionRecordIdByStudentIdAndCourseID(const Student::LogicID &stuId,
...
const Server::CourseSelectionRecordIdSet
Server::getCourseSelectionRecordIdByStudentId(const Student::LogicID &stuId) const
{
    auto iter = courseSelectionIndex.find(stuId);
    RunTimeAssert(iter != courseSelectionIndex.end(),
                  Docs::noCourseSelectionRecordForThisStudentError);
...
const Server::StudentIdSet
Server::getStudentLogicIdByClass(const Student::StudentClassLogicalID &classid) const
{
...
const Student::StudentClassName &
Server::getClassNameByClassID(const Student::StudentClassLogicalID &id) const
{
...
const CourseSelectionRecord &
Server::getCourseSelectionRecordByID(const CourseSelectionRecord::CourseSelectionRecordID &id) const
{
...
const Student::StudentClassLogicalID
Server::getClassLogicalIdByClassName(const Student::StudentClassName &className) const
{
