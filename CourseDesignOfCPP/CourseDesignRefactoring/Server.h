...
public:
    ...
    using CourseSelectionRecords =
      std::map<CourseSelectionRecord::CourseSelectionRecordID, CourseSelectionRecord>;
    ...
    using StudentNameIndex =
      std::map<Student::UserName, std::set<Student::LogicID>>;  //允许重名学生
    using CourseNameIndex = std::map<Course::CourseName, Course::CourseID>;
    using StudentIdIndex  = std::map<Student::ID, Student::LogicID>;
    using CourseSelectionIndex =
      std::map<Student::LogicID,
               std::map<Course::CourseID, CourseSelectionRecord::CourseSelectionRecordID>>;
    ...
    // login
    [[nodiscard]] CmdResalt loginAsStudent(const Student::ID & id,
                                           const Student::PWD &pwd) const;                //?
    ...
    [[maybe_unused]] CmdResalt addCourseSelectionRecord(const Student::LogicID &stuId,
    ...
    [[nodiscard]] CmdResalt setPassWordOfStudent(const Student::LogicID &studentId,
                                                 const Student::PWD &    newPwd);  //?
    [[nodiscard]] CmdResalt setCourseGrade(const Student::LogicID &                    stuId,
                                           const Course::CourseID &                    courseId,
                                           const CourseSelectionRecord::GradeOfCourse &grade);  //?
    [[nodiscard]] CmdResalt
    setCourseGrade(const CourseSelectionRecord::CourseSelectionRecordID csrid,
                   const CourseSelectionRecord::GradeOfCourse &         grade);  //?

    // remove
    [[nodiscard]] CmdResalt removeStudentByLogicId(const Student::LogicID &logicId);  //+
    [[nodiscard]] CmdResalt removeCourseSelectionRecord(const Student::LogicID &stuId,
                                                        const Course::CourseID &courseId);  //?
    ...
    [[nodiscard]] Server::IterPair<Server::CoursesRecord::const_iterator>
    getCoursesRecord(void) const;
    ...
    [[nodiscard]] const Student::StudentClassName &
                                               getClassNameByClassID(const Student::StudentClassLogicalID &id) const;  //?
    [[nodiscard]] const CourseSelectionRecord &getCourseSelectionRecordByID(
      const CourseSelectionRecord::CourseSelectionRecordID &id) const;  //?
    ...
    [[nodiscard]] const Server::StudentIdSet
    getStudentLogicIdByClass(const Student::StudentClassLogicalID &classid) const;  //+

    // get CourseId by Course Name
    [[nodiscard]] const Course::CourseID
    getCourseIdByCourseName(const Course::CourseName &courseName) const;  //+

    // get stuID by his or her name
    [[nodiscard]] const Server::StudentIdSet
    getStudentLogicIdByName(const Student::UserName &name) const;  //+
    ...
    [[nodiscard]] const Student::StudentClassLogicalID
    getClassLogicalIdByClassName(const Student::StudentClassName &className) const;  //+

    // get all the CSR this student has
    [[nodiscard]] const Server::CourseSelectionRecordIdSet
    getCourseSelectionRecordIdByStudentId(const Student::LogicID &stuId) const;  //?

    // get CSR_ID by student and course
    [[nodiscard]] const CourseSelectionRecord::CourseSelectionRecordID
    getCourseSelectionRecordIdByStudentIdAndCourseID(const Student::LogicID &stuId,
                                                     const Course::CourseID &courseId) const;  //?
private:
    // Section 1: data
    AdminRecord admin;
