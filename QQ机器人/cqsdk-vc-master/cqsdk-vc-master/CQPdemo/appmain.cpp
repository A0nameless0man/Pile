...
void NoThing(const char *msg, int64_t group, int64_t user)
{
    ...
    CQ_addLog(
      ...
      CQLOG_DEBUG,
      "unFinishedCOMMEND",
      ("user=" + to_string(user) + ";InGroup:" + to_string(group) + "withCOMMEND" + msg).c_str());
    replay += "\"";
    replay += msg;
    replay += "\"\n";
...
void Help(const char *msg, int64_t group, int64_t user)
{
    ...
    for(iter = Options.operate.begin(); iter != Options.operate.end(); iter++)
    {
        ...
        else
        {
            ...
            Reply += "\"	";
            Reply += "ID：";
            Reply += to_string(iter->second.ID);
            Reply += "\n";
        }
    }
...
void Start(const char *msg, int64_t groupid, int64_t user)
{
    ...
    else
    {
        ...
        for(set<member>::iterator i = iter->second.NameList.begin();
            i != iter->second.NameList.end();
            i++)
        {
...
(int32_t subType, int32_t msgId, int64_t fromQQ, const char *msg, int32_t font)
{
...
(int32_t subType, int32_t msgId, int64_t fromDiscuss, int64_t fromQQ, const char *msg, int32_t font)
{
    ...
    if (*msg == '#')
    {
            ...
            if (Options.operate.count(commend) == 1)
            {
                    ...
                    {
                            map<string, operate>::iterator iter;
                            for (iter = Options.operate.begin(); iter != Options.operate.end();
    iter++)
                            {
                                    ...
                                    if (iter->first == commend)
                                    {
                                            ...
                                            {
                                                    CQ_addLog(ac, CQLOG_DEBUG, "usefun",
    iter->first.c_str());
                                                    (*(iter->second.fun))(msg, fromGroup, fromQQ);
                                            }
                                            CQ_addLog(ac, CQLOG_DEBUG, "NULLfun",
    iter->first.c_str());
                                    ...
                                    {
                                            CQ_addLog(ac, CQLOG_DEBUG, "notusefun",
    iter->first.c_str());
                                    }
                            }
                    }
