			...
			for (caculate[1] = 0; caculate[1] < 4; caculate[1]++)
			{
				for (caculate[2] = 0; caculate[2] < 4; caculate[2]++)
				{
					if (caculate[3] == 0)
					{
						...
						if (fabs(easy(easy(input[0], input[1], caculate[0]), easy(input[2], input[3], caculate[1]), caculate[2]) - 24) <= 0.00005)
						{
					...
					{
						if (fabs(easy(easy(easy(input[0], input[1], caculate[0]), input[2], caculate[1]), input[3], caculate[2]) - 24) <= 0.00005)
						{
			...
			for (input[2] = 0; input[2] < 14; input[2]++)
			{
				for (input[3] = 0; input[3] < 14; input[3]++)
				{
					...
					if (trarange(input, range, caculate))
					{
						if (caculate[3] == 0)
						{
							printf("(%d%c%d)%c(%d%c%d)=24\n", range[0], sing[caculate[0]], range[1], sing[caculate[2]], range[2], sing[caculate[1]], range[3]);
						}
						else
						{
							printf("%d%c%d%c%d%c%d=24\n", range[0], sing[caculate[0]], range[1], sing[caculate[1]], range[2], sing[caculate[2]], range[3]);
						}
					}
					else
					{
