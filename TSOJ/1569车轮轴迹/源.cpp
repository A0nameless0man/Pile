...
int n, m, a, b, j, k;
LD r;
inline LD square(LD n) { return n * n; }
inline LD f(LD a, LD b) { return sqrtl(square(a) + square(b)); }
class Dot;
class Cir;
class Dir;
class Vec;
...
public:
	...
	template<class stream> friend stream& operator>>(stream& is, Dot& a)
	{
		return is >> a.x >> a.y;
	}
	friend LD distent(const Dot& a, const Dot& b) { return sqrtl(square(a.x - b.x) + square(a.y - b.y)); }
	LD x, y;
};
class Cir :virtual public Shape
{
public:
	virtual LD length(void) { return r * 2.0 * Pi; }
	Cir(const Dot& center, LD r) :center(center), r(r) { ; }
	Cir(const Dot& center, const Dot& second) :center(center), r(distent(center, second)) { ; }
	bool contain(const Dot& from)const { return r > distent(center, b); }
private:
	Dot center;
	LD r;
};
...
{
public:
	virtual LD length(void) { return INFINITY; }
...
{
public:
	virtual LD length(void) { return distent(from, to); }
	...
	friend Dot cross(const Lin& a, const Lin& b)
	{
		...
		//a1*x+b1*y==c1;
		//a2*x+b2*y==c2;
		return Dot((a1 * b2 - a2 * b1) / (b1 * c2 - b2 * c1), (a1 * b2 - a2 * b1) / (a1 * c2 - a2 * c1));
	}
private:
	Dot from, to;
};
...
public:
	...
	virtual LD length(void) { return (to - from) / (2 * Pi) * cir.length(); }
private:
	Cir cir;
	Dir from, to;
};
