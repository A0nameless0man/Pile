...
namespace bit
{
	...
	bool BigInt::operator==(const BigInt& b) const
	{
		...
		{
			if (postive == b.postive && ReverseInt.size()-PositionOfDot == b.ReverseInt.size()-b.PositionOfDot)
			{
				BigInt a(*this), b2(b);
				...
				{
					if (a.ReverseInt[i] != b2.ReverseInt[i])return false;
				}
				return true;
			}
			else
	...
	bool BigInt::absSmaller(const BigInt& b) const
	{
		if (ReverseInt.size()-PositionOfDot == b.ReverseInt.size()-b.PositionOfDot)
		{
			...
			{
				if (a.ReverseInt[i] < b2.ReverseInt[i])return true;
				if (b2.ReverseInt[i] < a.ReverseInt[i])return false;
			}
			return false;
		}
		else
