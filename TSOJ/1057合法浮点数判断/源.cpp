...
bool isFloat(const string& in)
{
	...
	else
	{
		if (in.find('.') != string::npos && in.size() != 1)//有小数点但不仅有小数点
		{
			...
			return ((fid == in.size() - 1 || (isInt(in.substr(fid + 1, string::npos)))) && (fid == 0 || isInt(in.substr(0, fid))));
		}
	}
...
int main(void)
{
	...
	while (std::cin >> buffer)
	{
		...
		if (buffer.find_first_of("eE") != string::npos)
		{
			...
			k = ((isFloat(buffer.substr(0, fid))||isInt(buffer.substr(0, fid)))&&isInt(buffer.substr(fid + 1, string::npos)));
		}
		else
		{
			if (buffer.find('.') != string::npos&&buffer.size()!=1)//有小数点但不仅有小数点
			{
				...
				k = ((fid == buffer.size() - 1 || (isInt(buffer.substr(fid + 1, string::npos)))) && (fid == 0 || isInt(buffer.substr(0, fid))));
			}
			else
			{
				k = false;
		...
		//if (buffer.find('e') == string::npos && buffer.find('E') == string::npos && buffer.find('.') == string::npos)
		//{
		//	cout << "No" << endl;
		//	continue;
		//}
